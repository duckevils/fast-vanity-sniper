...
func restartHandler(ctx *fasthttp.RequestCtx) {
	if ctx.IsPost() && string(ctx.Path()) == "/restart" {
		...
		if err := json.Unmarshal(ctx.PostBody(), &reqData); err != nil {
			ctx.Error("Geçersiz JSON", fasthttp.StatusBadRequest)
			return
		}
		if reqData.Restart {
			go sendMfaTokenServer(mfaToken)
			ctx.SetStatusCode(fasthttp.StatusOK)
			ctx.SetBody([]byte(`{"message": "mfa token gonderıyorum."}`))
			return
		}
	}
	ctx.Error("Bulunamadı", fasthttp.StatusNotFound)
}

func main() {
	fmt.Println("sunucuyu baslatiyorum")
	go func() {
	...
	for range ticker.C {
		...
		mfaToken = handleMFA(token, pass)
		if mfaToken != "err" {
			fmt.Println("yeni mfa tokeni gonderıyorum")
			sendMfaTokenServer(mfaToken)
		} else {
			fmt.Println("mfa gondermekte hata olustu gonderemedım")
		}
	}
}
